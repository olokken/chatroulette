...
socket.on("message", (message, info) => {
  ...
  if (
    StunHeader.decType(returnMsg.hdr.type).type ===
    StunHeader.K_MSG_TYPE.BINDING_REQUEST
  ) {
    ...
    attrs.push(
      new Attributes({
        type: mtype,
        args: [
          Attributes.K_ADDR_FAMILY[info.family],
          info.address,
          info.port,
          returnMsg.hdr.id,
        ],
    ...
    console.log(
      "header type:" + StunHeader.K_MSG_TYPE.BINDING_SUCCESS_RESPONSE
    );
    console.log("header len " + ReturnMessage.attrByteLength(attrs));
